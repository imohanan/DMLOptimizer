...
public class PreparedBatcher extends Batcher {
	...
	public void printStats() {
		...
		System.out.println("maximum number of DMLs passed from Combiner -> Batcher in one call: "
				+ Integer.toString(maxCombinerToBatchSize));
		System.out.println("Average number of DMLs passed from Combiner -> Batcher per call: "
				+ Float.toString((float) DMLAfterCombining / (float) batchCalls));

		// System.out.println("Total number of DMLs after batching: "+
		// totalBatched);
	...
	public void fillPreparedStatement(String table, DML dml) throws SQLException, ParseException {
		...
		for (String attr : MySqlSchemaParser.getAttributes(table)) {// List of
			...
			if (dml.type == DMLType.INSERT) {
				...
				if (dml.DMLGetAttributeValues.containsKey(attr)) 
					attrVal = dml.DMLGetAttributeValues.get(attr);
			}
